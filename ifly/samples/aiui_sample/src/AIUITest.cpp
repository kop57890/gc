...
void AIUITester::readCmd(){
	...
	while (true){
		...
		string result = "";
		debug_log(__FUNCTION__, __LINE__, "while loop count: " + Int_to_String(count));
		writeText(first);
		while(state != 1){
			usleep(500000);
		}
		if(count == 0){
			...
			if(strstr(newline, "结束") != NULL){
				debug_log(__FUNCTION__, __LINE__, "结束 -> go back to awaken status");
				break;
				...
				if(count_null >= 2){
					...
					state = 0;
					debug_log(__FUNCTION__, __LINE__, "No Speak input -> 请问这是什么材料构成的");
					writeText(sec);
				}
				count_null++;
			}else{
				state = 0;
				count_null = 0;
				debug_log(__FUNCTION__, __LINE__, "Speak:" + result);
				writeText(result);
			}
			pclose(fd);
		}else{
			debug_log(__FUNCTION__, __LINE__, "Error");
		}
		while(state != 1){
			usleep(500000);
		}
